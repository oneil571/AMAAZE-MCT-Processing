...
scan_num = 1#int(os.getcwd().split('/')[-1].split('_')[-1])
...
for i in range(n_tiers):
    ...
    scan_layout.append(s)
    mask.append(m)
    
    tier_mask.append(np.sum(m)>0)
...
q = -np.sum(np.sum(vol,1),1)
sig = q.copy()
q = q-q.min()
    
thresh = vol.shape[1]*vol.shape[2]*(3e7/(225**2))
q[q<thresh] = 0
...
for i in range(len(tiers)):
    ...
    if AUTO_ROT:
        ...
        angi,Im = autorot2(si,t1=t1t2t3[0],t2=t1t2t3[1],t3=t1t2t3[2],frac_thresh = .4) 
        #Im =   rotate(Im,angi,preserve_range=True)
        
    else:
        ''' same params go into autorot2:'''
        Im = id_cardboard(si,t1=t1t2t3[0],t2=t1t2t3[1],t3=t1t2t3[2],frac_thresh = .4)
        angi = 0
    ...
    if AUTO_SEG==True:
        x0 = auto_seg(Isum0,dim2-1)
        x1 = auto_seg(Isum1,dim1-1)
    ...
    plt.figure()
    plt.plot(np.arange(len(Isum0)),Isum0)
    plt.plot(np.arange(len(Isum0)),x0*Isum0.max())
    plt.show()
    
    plt.figure()
    plt.plot(np.arange(len(Isum1)),Isum1)
    ...
    if USE_SSH & SCPPLOT:
        draw_boxes(imdisp,drawrow,drawcol)
