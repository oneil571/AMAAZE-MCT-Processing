...
if __name__ == '__main__':
    ...
    if USE_SSH:
        print('please configure SSH and enter your password in line 28')
        ssh = createSSHClient('server.address',22,'username', 'password')
        scp = SCPClient(ssh.get_transport())

    os.chdir(opt.folder)
    scan_num = int(os.getcwd().split('/')[-1].split('_')[-1])

    slicepath_ssh = './amaazescans/CT_'+str(scan_num)+'/Slices/AMAAZE_'+str(scan_num)+' Y Slices/'
    slicepath_local = '/Slices/AMAAZE_'+str(scan_num)+' Y Slices/'
    ...
    for i in range(len(tiers)):
        ...
        if (USE_SSH | LOCAL_SLICES) & PLOTTING :
